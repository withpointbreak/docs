...
- **Option A** for GitHub Copilot or Cursor (simplest - no CLI needed)
- **Option B** for Claude Code (requires CLI installation)
- **Option C** for other MCP clients
</Note>

### Option A: GitHub Copilot or Cursor (Built-in)

<Tip>
If you're using GitHub Copilot or Cursor, you're done! No additional setup required—skip to Step 4.
</Tip>
...
### Option B: Claude Code (External AI Assistant)

<Warning>
**CLI installation required!** Claude Code won't work with Pointbreak unless you install both the extension AND the CLI.
</Warning>
...
2. Install Pointbreak CLI:
   ...
   ```
3. Add Pointbreak as an MCP server:
   ```bash
   claude mcp add --transport stdio pointbreak -- pointbreak
   ```

For more details, see [Claude Code Setup](/ai-assistants#claude-code)

### Option C: Other MCP Clients (Codex, Windsurf, etc.)
...
<Tip>
Look for the Pointbreak status indicator in your VS Code status bar (bottom of window). A green status means everything is working!
</Tip>
